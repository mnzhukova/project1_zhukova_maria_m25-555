...
def move_player(game_state, direction):
    ...
    Функция проверяет наличие выхода в заданном направлении из текущей комнаты, 
    обрабатывает специальные условия (например, запертую дверь в комнату сокровищ) 
    и обновляет состояние игры при успешном перемещении.
    ...
    Returns:
        bool: 
            - True: перемещение успешно выполнено;
            - False: перемещение невозможно (нет выхода или препятствие)
    '''
...
def use_item(game_state, item_name):
    '''
    Использует указанный предмет из инвентаря игрока.

    Функция проверяет наличие предмета в инвентаре, выполняет соответствующие 
    действия в зависимости от типа предмета и выводит информативные сообщения.
    ...
    Возвращаемое значение:
        bool:
            - True: предмет успешно использован;
            - False: предмет отсутствует в инвентаре.
    ''' 
